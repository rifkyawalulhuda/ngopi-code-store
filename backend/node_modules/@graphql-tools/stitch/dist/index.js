import { handleOverrideByDelegation, subtractSelectionSets, extractUnavailableFields, leftOverByDelegationPlan, extractUnavailableFieldsFromSelectionSet, delegateToSchema, cloneSubschemaConfig, isSubschemaConfig, defaultMergedResolver, Subschema } from '@graphql-tools/delegate';
...
function getFieldsNotInSubschema(schema, stitchingInfo, gatewayType, subschemaType, fieldNodes, fragments, variableValues, subschema, providedSelectionNode, context, info) {
  ...
  for (const [, subFieldNodes] of subFieldNodesByResponseKey) {
    ...
    let field = fieldName === "__typename" ? TypeNameMetaFieldDef : fields[fieldName];
    if (context != null && info != null) {
      const overrideHandler = subschema?.merge?.[gatewayType.name]?.fields?.[fieldName]?.override;
    ...
    if (!field) {
      ...
      typeof globalThis === "undefined" || // @ts-expect-error
      !globalThis["__DO_NOT_USE__stitching_disable_extract_unavailable_fields_for_fields_not_in_schema__"]
    ) {
      for (const subFieldNode of subFieldNodes) {
...
function calculateDelegationStage(mergedTypeInfo, sourceSubschemas, targetSubschemas, fieldNodes, fragments, context, info) {
  ...
  for (const fieldNode of fieldNodes) {
    ...
    let existingSubschema = nonUniqueSubschemas.find(
      (s) => delegationMap.has(s)
    );
    let overridden = false;
    for (const nonUniqueSubschema of nonUniqueSubschemas) {
      if (context != null && info != null) {
        const overrideHandler = nonUniqueSubschema.merge?.[mergedTypeInfo.typeName]?.fields?.[fieldNode.name.value]?.override;
        if (overrideHandler != null) {
          ...
          if (overriddenBySubschema) {
            ...
            subschemaSelections.selections.push(fieldNode);
            overridden = true;
            break;
          } else {
            existingSubschema = void 0;
    ...
    if (existingSubschema != null) {
      ...
      for (const nonUniqueSubschema of nonUniqueSubschemas) {
        ...
        if (field != null) {
          if (context != null && info != null) {
            const overrideHandler = nonUniqueSubschema.merge?.[mergedTypeInfo.typeName]?.fields?.[fieldNode.name.value]?.override;
            if (overrideHandler != null) {
              const overridden2 = handleOverrideByDelegation(
                info,
                context,
                overrideHandler
              );
      ...
      if (existingSelections != null) {
        existingSelections.selections.push(fieldNode);
      } else {
...
function createDelegationPlanBuilder(mergedTypeInfo) {
  mergedTypeInfo.nonMemoizedDelegationPlanBuilder = function delegationPlanBuilder(schema, sourceSubschema, variableValues, fragments, fieldNodes, context, info) {
        ...
        for (const typeCandidate2 of typeCandidatesOfTypeName) {
          ...
          if (mergedTypeConfig2.fields) {
            ...
            for (const fieldName in mergedTypeConfig2.fields) {
              if (mergedTypeConfig2.fields[fieldName]?.override) {
                hasOverrides = true;
              }
...
function filterBaseSubschema(subschemaConfig, isolatedSchemaTypes) {
  ...
  const filteredSchema = filterSchema({
    ...
    objectFieldFilter: (typeName, fieldName) => {
      ...
      const isIsolatedFieldName = allTypes.some(

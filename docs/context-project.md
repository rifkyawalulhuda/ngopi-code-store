...
Headless e-commerce platform for selling digital products (source code, ebooks, templates) to Indonesian developers. Guest checkout, Tripay payment gateway, automated digital fulfillment, customer accounts with email verification + Google OAuth.
...
| OAuth | Google (custom plugin via google-auth-library) |
...
| `backend/src/plugins/google-auth/` | Google OAuth authentication (verifies ID token, creates/finds customer) |
...
| File | Purpose |
|------|---------|
| `frontend/nuxt.config.ts` | Nuxt config (Apollo, Pinia, SSR, devServer port 3001, Google Identity Services script) |
...
| `frontend/pages/auth/index.vue` | Login/Register (tabs, confirm-password, Google OAuth, redirects logged-in → /account) |
...
| `frontend/composables/useAuth.ts` | Shared auth state (useState): register/login/loginWithGoogle/logout/verify/updateProfile/changePassword/email-change |
...
| `frontend/graphql/mutations/auth.ts` | REGISTER, LOGIN, LOGOUT, VERIFY_CUSTOMER, UPDATE_CUSTOMER, UPDATE_CUSTOMER_PASSWORD, REQUEST_UPDATE_EMAIL, UPDATE_EMAIL_ADDRESS, GET_ACTIVE_CUSTOMER(_ORDERS), AUTHENTICATE_WITH_GOOGLE |
...
→ findCustomerUser or createCustomerAndUser → logged in (no email verification needed)
```
- `requireVerification: true`, token valid 7 days (native auth only)
- Google OAuth users auto-verified based on Google's email_verified status
...
# Env (backend/.env):
#   GOOGLE_CLIENT_ID, RESEND_API_KEY, EMAIL_FROM_ADDRESS=noreply@info.ngopidulur.my.id,
#   EMAIL_FROM_NAME, STOREFRONT_URL=http://localhost:3001
# Env (frontend/.env):
#   NUXT_PUBLIC_GOOGLE_CLIENT_ID (same value as backend GOOGLE_CLIENT_ID)
```

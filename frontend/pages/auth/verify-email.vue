    ...
    <main class="verify-main">
      <div class="verify-card">
        <div v-if="loading" class="verify-state">
          ...
          <h2>Memverifikasi email baru...</h2>
        ...
        <div v-else-if="verified" class="verify-state">
          ...
          <h2>Email Berhasil Diubah!</h2>
          <p>Alamat email akun kamu sudah diperbarui. Gunakan email baru untuk login berikutnya.</p>
          <NuxtLink to="/account" class="btn btn-primary">Ke Akun Saya</NuxtLink>
        </div>
        ...
        <div v-else class="verify-state">
          ...
          <p>{{ errorMessage }}</p>
          <NuxtLink to="/account" class="btn btn-primary">Kembali ke Akun</NuxtLink>
        </div>
      </div>
    </main>
...
useHead({
  title: 'Verifikasi Email Baru - NgopiCode',
})

const route = useRoute()
const { confirmEmailChange, error: authError } = useAuth()
...
onMounted(async () => {
  ...
  if (!token) {
    loading.value = false
    errorMessage.value = 'Token verifikasi tidak ditemukan. Pastikan menggunakan link dari email.'
    return
  }

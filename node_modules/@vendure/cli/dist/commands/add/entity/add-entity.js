...
async function addEntity(options) {
    ...
    if ((options === null || options === void 0 ? void 0 : options.pluginName) && !vendurePlugin) {
        const pluginClasses = (0, ast_utils_1.getPluginClasses)(project);
        const pluginClass = pluginClasses.find((p) => p.getName() === options.pluginName);
        if (!pluginClass) {
            const availablePlugins = pluginClasses.map((p) => p.getName()).join(', ');
            throw new Error(`Plugin "${options.pluginName}" not found. Available plugins: ${availablePlugins}`);
        }
        vendurePlugin = new vendure_plugin_ref_1.VendurePluginRef(pluginClass);
    }
    if ((options === null || options === void 0 ? void 0 : options.isNonInteractive) && !vendurePlugin) {
        throw new Error('Plugin must be specified when running in non-interactive mode. Usage: vendure add -e <entity-name> --selected-plugin <plugin-name>');
    }
...
async function getFeatures(options) {
    ...
    if (options === null || options === void 0 ? void 0 : options.isNonInteractive) {
        return {
            customFields: (_a = options === null || options === void 0 ? void 0 : options.customFields) !== null && _a !== void 0 ? _a : false,
            translatable: (_b = options === null || options === void 0 ? void 0 : options.translatable) !== null && _b !== void 0 ? _b : false,
        };
    }
    if ((options === null || options === void 0 ? void 0 : options.className) && !(options === null || options === void 0 ? void 0 : options.features)) {
        return {
            customFields: true,
            translatable: false,
        };
    }
    const features = await (0, utils_1.withInteractiveTimeout)(async () => {
...
function createEntity(plugin, options) {
    ...
    if (!options.features.translatable) {
        ...
        (_c = entityFile.getImportDeclaration('./entity-translation.template')) === null || _c === void 0 ? void 0 : _c.remove();
    }
    else {
        (_d = entityFile
            .getImportDeclaration('./entity-translation.template')) === null || _d === void 0 ? void 0 : _d.setModuleSpecifier(`./${options.translationFileName}`);
        (_e = translationFile
            .getImportDeclaration('./entity.template')) === null || _e === void 0 ? void 0 : _e.setModuleSpecifier(`./${options.fileName}`);
    }

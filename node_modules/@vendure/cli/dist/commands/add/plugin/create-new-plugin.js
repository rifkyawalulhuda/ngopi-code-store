...
exports.createNewPluginCommand = new cli_command_1.CliCommand({
    ...
    run: (options) => createNewPlugin(options),
});
const cancelledMessage = 'Plugin setup cancelled.';
async function createNewPlugin(options = {}) {
    ...
    const { project, config, vendureTsConfig } = await (0, shared_prompts_1.analyzeProject)({
        cancelledMessage,
    ...
    const vendureProjectDir = vendureTsConfig ? path_1.default.dirname(vendureTsConfig) : process.cwd();
    ...
    const vendureConfig = new vendure_config_ref_1.VendureConfigRef(project, config);
    ...
    const followUpCommands = [
        ...
        add_dashboard_1.addDashboardCommand,
    ...
    while (!done) {
        const featureType = await (0, utils_1.withInteractiveTimeout)(async () => {
            ...
            return await (0, prompts_1.select)({
                message: `Add features to ${(_a = options.name) !== null && _a !== void 0 ? _a : 'plugin'}?`,
                options: [
        ...
        if ((0, prompts_1.isCancel)(featureType) || featureType === 'no') {
            done = true;
        }
        else {
...
function getPluginDirName(name, existingPluginDirPattern, vendureProjectDir) {
    ...
    if (existingPluginDirPattern) {
        ...
        return path_1.default.join(vendureProjectDir, 'src', 'plugins', (0, change_case_1.paramCase)(nameWithoutPlugin));
    }
}

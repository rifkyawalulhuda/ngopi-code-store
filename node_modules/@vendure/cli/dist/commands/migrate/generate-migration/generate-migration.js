...
const load_vendure_config_file_1 = require("../../../shared/load-vendure-config-file");
...
exports.generateMigrationCommand = new cli_command_1.CliCommand({
    ...
    run: options => runGenerateMigration(options === null || options === void 0 ? void 0 : options.configFile),
});
async function runGenerateMigration(configFile) {
    const { project, tsConfigPath } = await (0, shared_prompts_1.analyzeProject)({ cancelledMessage });
    const vendureConfig = new vendure_config_ref_1.VendureConfigRef(project, configFile);
    prompts_1.log.info('Using VendureConfig from ' + vendureConfig.getPathRelativeToProjectRoot());
    const name = await (0, utils_1.withInteractiveTimeout)(async () => {
    ...
    if (migrationsDirs.length > 1) {
        const migrationDirSelect = await (0, utils_1.withInteractiveTimeout)(async () => {
    ...
    if (migrationsDirs.length === 1 || migrationDir === 'other') {
        const confirmation = await (0, utils_1.withInteractiveTimeout)(async () => {
            return await (0, prompts_1.text)({
                message: 'Migration file location',
